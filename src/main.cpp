...
DomainWallFermion<SpWilsonImplD>::FermionField testDwfMatrix_space(GridCartesian &GRID, GridRedBlackCartesian &RBGRID, SpFundamentalRepresentation::LatticeField &Umu, const std::vector<int>& seeds, const int vol, const Coordinate latt_size, GridSerialRNG sRNG, GridParallelRNG pRNG)
{
...
int main(int argc, char **argv)
{
    ...
    std::cout << "\n\n\n\n" << std::endl;
    //Return free field propagators obtained from a random gauge configuration
    auto xform_prop = testDwfMatrix_space(GRID, RBGRID, Urnd, seeds, vol, latt_size, sRNG, pRNG);

    std::cout << std::fixed << std::setprecision(26);
    std::cout << "\n Squared norm of the difference between unit and random free field propagators\n";
    std::cout << "unit - xform     "
